...
class CStatsWindDirection
{
public:
	void Update(double data)
	{
		m_sin += std::sin(ToRadiands(data));
		m_cos += std::cos(ToRadiands(data));
		m_average = std::fmod(ToDegress(std::atan2(m_sin, m_cos)), 360);
	}
	std::string DiplayStats()
	{
		return "Average Direction: " + std::to_string(m_average);
	}

private:
	double ToRadiands(double degress)
	{
...
public:
	void UpdateStats(const SWeatherInfo& data)
	{
		...
		m_speed.Update(data.wind.speed);
		m_wind.Update(data.wind.direction);
	}
	void DisplayStats(std::ostream& out)
	{
		...
		out << m_speed.DisplayStats("speed") << std::endl;
		out << m_wind.DiplayStats() << std::endl;
	}
private:
...
{
public:
	CStatsDisplay(const CWeatherData& observerIn, const CWeatherDataPro& observerOut, std::ostream& output)
...
private:
	...
	*/
	...
	const CWeatherDataPro& m_observerOut;
