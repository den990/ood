...
{
public:
	double GetMinTemperature() const { return m_temperature.minValue; }
	double GetMaxTemperature() const { return m_temperature.maxValue; }
	double GetMinHumidity() const { return m_humidity.minValue; }
	double GetMaxHumidity() const { return m_humidity.maxValue; }
	double GetMinPressure() const { return m_pressure.minValue; }
	double GetMaxPressure() const { return m_pressure.maxValue; }
private:
	/* Метод Update сделан приватным, чтобы ограничить возможность его вызова напрямую
	Классу CObservable он будет доступен все равно, т.к. в интерфейсе IObserver он
	остается публичным
